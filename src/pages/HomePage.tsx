...
/**
 * Each category card has a name, description, representative search keyword,
 * and an Unsplash image that fits the theme.
 */
const categories = [
  {
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-3">
            {categories.map((cat) => (
              <button
                key={cat.name}
                onClick={() => navigate(`/search?q=${encodeURIComponent(cat.keyword)}`)}
